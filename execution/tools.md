...
| Seedance 2.0 i2v | `bytedance/seedance-2.0/image-to-video` | **Animazione di una statica** (`56_animate_static`) | supporta **start+end frame control** — è ciò che fa atterrare il video esattamente sulla statica |
...
> **Modelli con start+end frame** (servono a `56_animate_static`, dove l'ultimo frame DEVE essere la statica): al 2026-08 su fal.ai lo supportano `bytedance/seedance-2.0/image-to-video` (+ tier `fast`/`mini`), `fal-ai/kling-video/o3/*/image-to-video`, `minimax/h3/image-to-video`, `fal-ai/bytedance/seedance/v1.5/pro/image-to-video`. Verifica lo schema con `mcp__fal-ai__find` prima di chiamare (i nomi campo cambiano). Se il modello scelto NON espone l'end-frame, dillo all'utente: senza quel controllo la skill perde il suo punto.
