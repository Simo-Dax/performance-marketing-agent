---
description: ASMR UGC video ad (15-25s). I suoni e le texture del prodotto reggono tutta l'ad. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-asmr — ASMR

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **asmr**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/asmr/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/asmr/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** **Funziona meglio su brand già riconosciuti** — misura negativamente per quelli sconosciuti. Va detto all'utente, non è un motivo per rifiutare. È il render più affidabile del set.
...
Output: `05_UGC_Prompts/formats/asmr/<concept>/`
