---
description: Founder Story UGC video ad (24-30s). Il fondatore racconta perché questa cosa esiste. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-founder-story — Founder Story

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **founder-story**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/founder-story/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/founder-story/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** **Richiede una foto vera del fondatore.** Il fondatore non si genera mai. Costruisce brand, non vende un pezzo.
...
Output: `05_UGC_Prompts/formats/founder-story/<concept>/`
