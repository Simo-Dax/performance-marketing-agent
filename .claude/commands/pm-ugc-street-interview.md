---
description: Street Interview UGC video ad (22-30s). Sconosciuti in pubblico, risposte non copionate, nessun interesse in gioco. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-street-interview — Street Interview

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **street-interview**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/street-interview/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/street-interview/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** **Richiede la VOC.** Le risposte vengono da linguaggio cliente reale — mai inventate.
...
Output: `05_UGC_Prompts/formats/street-interview/<concept>/`
