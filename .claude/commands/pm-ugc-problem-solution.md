---
description: Problem/Solution UGC video ad (20-27s). Nomina un dolore, mostralo fallire, poi il prodotto che lo chiude. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-problem-solution — Problem/Solution

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **problem-solution**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/problem-solution/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/problem-solution/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** Il dolore va nominato, non alluso: viene dalla VOC o dall'insight, mai inventato.
...
Output: `05_UGC_Prompts/formats/problem-solution/<concept>/`
