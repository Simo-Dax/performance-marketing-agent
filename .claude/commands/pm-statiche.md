...
2. **Reference ads obbligatorie**: la skill scrapa gli ad live del brand + legge la reference bank bancata da `19_ad_spy` (`03_Ad_Spy/_scratch/format-*.json`). Zero fonti + scan competitor rifiutato = si ferma e indirizza a `/pm-competitor-spy` — a meno che manchi anche un concept SA5, nel qual caso offre il **Percorso Z** (fallback a template legacy, solo su scelta esplicita dell'utente, vedi Step Z della skill).
