---
description: Testimonial UGC video ad (20-30s). Un cliente vero dice cosa è cambiato, parole sue, in camera. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-testimonial — Testimonial

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **testimonial**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/testimonial/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/testimonial/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** **Richiede la VOC.** Le parole vengono da linguaggio cliente reale — mai inventate. Senza VOC → prima `/pm-dati-qualitativi`.
...
Output: `05_UGC_Prompts/formats/testimonial/<concept>/`
