---
description: Expert / Authority UGC video ad (22-30s). Qualcuno competente spiega **perché** funziona, non *che* funziona. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-expert — Expert / Authority

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **expert**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/expert/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/expert/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** **Richiede la VOC.** Spiega il meccanismo, non il beneficio. Attenzione ai claim: passa da `_shared/creative_claims_compliance.md`.
...
Output: `05_UGC_Prompts/formats/expert/<concept>/`
