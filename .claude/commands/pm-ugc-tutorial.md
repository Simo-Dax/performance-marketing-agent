---
description: Tutorial UGC video ad (22-30s). Insegna UN risultato, step by step, col prodotto come strumento. Skill nativa 62_ugc_formats (SA6).
argument-hint: [prodotto/concept] — opzionale, altrimenti parte dal Brand DNA e dalla VOC
---

# /pm-ugc-tutorial — Tutorial

Esegui la skill nativa **`directives/skills/62_ugc_formats/SKILL.md`** (SA6 — Asset Production), formato **tutorial**.

Argomenti: $ARGUMENTS

## Cosa fare
1. Carica il playbook del formato: `directives/skills/62_ugc_formats/formats/tutorial/playbook.md`, più `format-spec.md` e `shot-vocabulary.md` della stessa cartella.
2. Le 10 coppie di studio stanno in `formats/tutorial/recreation-prompts/` — `anchor/` (fermo immagine) e `video/` (movimento). Sono ad **reali** di questo formato, smontate shot by shot. Banco di studio, non template da copiare.
...
5. **Specifico di questo formato:** Un solo risultato per ad. Se ne insegni due, non se ne ricorda nessuno.
...
Output: `05_UGC_Prompts/formats/tutorial/<concept>/`
