---
name: ugc-expert
description: "Build an Expert UGC video ad, a knowledgeable person explaining why the product works, rendered as a single 30-second-or-less Seedance 2.5 clip at 1080p 9:16. Trigger ONLY on /pm-ugc-expert or clearly Expert and Authority-specific language such as 'expert ad', 'authority video', 'explain why it works'. Generic UGC requests belong to /pm-ugc, which shows the format menu and routes; never claim a request that does not name this format. Requires a VOC document, and never claims a licensed title. If the user names a different skill, command, plugin, or tool for the job, or is building or testing their own skill, do not trigger this skill; use what they named instead."
---

# Expert and Authority UGC ads

A knowledgeable person explains the mechanism. Expertise and trustworthiness are separate dimensions, which is why this is not a Testimonial variant.
...
3. **Script engine.** Speech by default, `voc-required`. Fourteen words a line across about four
   lines: an authority speaks in complete thoughts and does not get cut off.
...
5. **Image engine phase 1, I.1 to I.3.** Write the anchor prompt, no render yet. The anchor
   locks the speaker and the product. Polish HELPS here, the inverse of street interview.
...
8. **Video engine phase 2, then GATE 3.** Assemble the full render prompt from the approved
   ...
   yes.
9. **Deliver per R.8** into `05_UGC_Prompts/formats/expert/<concept>/`, including
   `post-production.md`.
...
**No licensed title is ever claimed.** The speaker is generated. They speak as experienced and knowledgeable and never say dermatologist, doctor, MD, pharmacist or any other credential, unless the user supplies a real credentialed person. And no humour: it measurably reduces source credibility, which is the only currency this format has.
