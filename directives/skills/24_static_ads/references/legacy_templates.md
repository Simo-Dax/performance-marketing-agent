> ⚠️ **Percorso Z (fallback legacy) — solo ultima spiaggia.** Vedi la nota in `legacy_visual_families.md` / "Percorso Z" in `SKILL.md`.

# 40 Static Ad Template Prompts

These are the exact templates to fill in during Step 2 of the workflow. Use the Brand DNA document, VOC research (if provided), and product images (if provided) to fill every placeholder. The finished prompts are model-agnostic, they paste cleanly into either GPT Image 2 (recommended default at high quality and 4K equivalent `image_size`) or Nano Banana 2 (cheaper alternative).

The prompt to run is:

---

Now take this list of prompt templates and fill them in with detail for each one that needs context specifically for [BRAND NAME] that aligns with the brand dna information/doc, especially the image generation prompt modifier at the end of the document and the ad creative section guidelines.
...
Please format me a new doc with finished prompts for each one that I can plug directly into my image model (GPT Image 2 by default or Nano Banana 2 as the cheaper alternative) for this product - [SPECIFIC PRODUCT] (images attached). Make sure that each prompt is its own standalone prompt that is immediately ready to drop into the model to create a high converting image that can be plugged into an ad account for [BRAND NAME] with no edits needed.
