> ⚠️ **Percorso Z (fallback legacy) — solo ultima spiaggia.** Il flusso primario di `24_static_ads` è il rebrand da reference bank (vedi SKILL.md). Questo file alimenta il Percorso Z, usato SOLO quando la reference bank è vuota, l'utente rifiuta lo scan competitor, e non esiste un concept SA5. Vedi "Percorso Z" in `24_static_ads/SKILL.md` per il trigger esatto.

# Visual families e il render prompt template-structured

Questo file definisce le **5 visual families fisse** e la struttura di ogni render prompt. Il Percorso Z di `24_static_ads` carica questo file insieme a `legacy_templates.md` e scrive esattamente 5 prompt per angolo, uno per famiglia. Ogni prompt è costruito su un template chiamato per nome e mira a `$RENDER_MODEL` (GPT Image 2 di default; Nano Banana 2 solo per un 4:5 vero insistito).
...
Ogni prompt è costruito su un template per nome da `legacy_templates.md`. La famiglia è il **job di persuasione** che la variante deve fare; il template è la **struttura on-image concreta** che lo realizza. Costruisci ogni prompt in quest'ordine.
