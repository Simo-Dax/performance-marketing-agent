#!/usr/bin/env python3
"""
seo_checker.py — Analyzer SEO on-page
Uso:
  python3 seo_checker.py [--file page.html] [--url https://...] [--json]
  python3 seo_checker.py          # modalità demo con HTML di esempio incorporato
"""
...
class SEOParser(HTMLParser):
    def __init__(self):
        ...
        self.h_tags = []          # lista di (livello, testo)
        self._current_h = None
        self._current_h_text = []
        self.images = []          # lista di {"src": ..., "alt": ...}
        self._in_body = False
        self.links = []           # lista di {"href": ..., "text": ...}
...
def analyze_html(html: str, base_domain: str = "") -> dict:
    ...
    results["title"] = {
        "value": title,
        "length": title_len,
        "optimal_range": "50-60 caratteri",
        "pass": title_ok,
        "score": 100 if title_ok else (50 if title else 0),
        "note": "Lunghezza buona" if title_ok else (
            f"Troppo {'corto' if title_len < 50 else 'lungo'} ({title_len} caratteri)" if title else "Title tag mancante"
        ),
    }
    ...
    results["meta_description"] = {
        ...
        "length": desc_len,
        "optimal_range": "150-160 caratteri",
        "pass": desc_ok,
        "score": 100 if desc_ok else (50 if 100 <= desc_len < 150 or 160 < desc_len <= 200 else (30 if desc else 0)),
        "note": "Lunghezza buona" if desc_ok else (
            f"Troppo {'corta' if desc_len < 150 else 'lunga'} ({desc_len} caratteri)" if desc else "Meta description mancante"
        ),
    }
    ...
    results["h1"] = {
        ...
        "note": "Esattamente un H1 ✓" if h1_ok else (
            f"H1 multipli ({h1_count})" if h1_count > 1 else "Nessun H1 trovato"
        ),
    }
    ...
    for lvl, _ in parser.h_tags:
        if prev_level and lvl > prev_level + 1:
            heading_issues.append(f"H{prev_level} → H{lvl} salta un livello")
        prev_level = lvl
    ...
    results["heading_hierarchy"] = {
        ...
        "note": "Gerarchia OK" if hierarchy_ok else f"{len(heading_issues)} problema/i di salto livello",
    }
    ...
    results["image_alt_text"] = {
        ...
        "pass": alt_ok,
        "score": round(alt_pct),
        "note": "Tutte le immagini hanno alt text" if alt_ok else f"{total_imgs - imgs_with_alt} immagine/i senza alt",
    }
    ...
    results["link_ratio"] = {
        ...
        "note": "Buon bilanciamento internal/external" if ratio_ok else "Più link external che internal",
    }
    ...
    results["word_count"] = {
        ...
        "note": f"{word_count} parole (buono)" if wc_ok else f"Solo {word_count} parole — servono 300+",
    }
    ...
    results["viewport_meta"] = {
        ...
        "note": "Tag viewport mobile presente" if parser.viewport_meta else "Meta tag viewport mancante",
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="SEO checker on-page — assegna a una pagina HTML un punteggio 0-100."
    )
    parser.add_argument("--file", help="Path al file HTML")
    parser.add_argument("--url", help="URL da scaricare e analizzare")
    parser.add_argument("--domain", default="", help="Dominio base per il rilevamento dei link interni")
    ...
    if args.file:
        ...
        html = DEMO_HTML
        if not args.json:
            print("Nessun input fornito — esecuzione in modalità demo.\n")
    ...
    ICONS = {True: "✅", False: "❌"}
    print("=" * 60)
    print(f"  RISULTATI SEO AUDIT   Punteggio Complessivo: {overall}/100")
    print("=" * 60)
    ...
    checks = [
        ...
        ("H1 Tag",              "h1"),
        ("Gerarchia Heading",   "heading_hierarchy"),
        ("Alt Text Immagini",   "image_alt_text"),
        ("Rapporto Link",       "link_ratio"),
    ...
    print(f"  Voto: {grade}   Punteggio: {overall}/100")
    print("=" * 60)
